...
#define LOGI(...) __android_log_print(ANDROID_LOG_INFO, "imageutils_jni", ##__VA_ARGS__);
#define LOGE(...) __android_log_print(ANDROID_LOG_ERROR, "imageutils_jni", ##__VA_ARGS__);


#define IMAGEUTILS_METHOD(METHOD_NAME) \
  Java_com_qtimes_utils_android_ImageUtils_##METHOD_NAME  // NOLINT
...
#if 1
        ConvertYUV420SPToARGB8888Neon2(reinterpret_cast<uint8_t *>(i), width, height,
                                       reinterpret_cast<uint32_t *>(o));
#else
        LOGI("ConvertYUV420SPToARGB8888-----------------------");
