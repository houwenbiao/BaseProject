...
public class ZipUtil {
     ...
     * 使用第三方工具类压缩文件
     *
     * @param command 7z a archive1.zip subdir\
     * @return
     */
    public static int zipFolder(String command) {
        return P7ZipApi.executeCommand(command);
    }
     ...
     * @param zipFilePath 指定压缩的目录和名字
     * @throws Exception
     */
    public static void zipFolder(String srcFilePath, String zipFilePath) throws Exception {

        FileOutputStream stream = new java.io.FileOutputStream(zipFilePath);

        //创建Zip包
        java.util.zip.ZipOutputStream outZip = new java.util.zip.ZipOutputStream(stream);

        //打开要输出的文件
        java.io.File file = new java.io.File(srcFilePath);

        PluLog.i("Start zip files");
        //压缩
