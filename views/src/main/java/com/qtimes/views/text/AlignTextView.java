...
public class AlignTextView extends TextView {
    ...
    private String userName = "";//用户名称：这里变色
    private int userNameColor = Color.GREEN;//用户名称颜色
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            for (int j = 0; j < line.length(); j++) {
                String drawText = line.substring(j, j + 1);
                paint.setColor(defaultColor);
                if (i == 0 && num < userName.length() && drawText.equals(userName.substring(num, num + 1))) {//第一行
                    Log.e("tag", drawText);
                    paint.setColor(userNameColor);
                    num++;
                }
                float drawX = paint.measureText(line.substring(0, j)) + interval * j;
                canvas.drawText(drawText, drawX + drawSpacingX, drawY + paddingTop + textLineSpaceExtra * i, paint);
            }
        }
    }
