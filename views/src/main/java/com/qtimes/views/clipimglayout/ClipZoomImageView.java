...

{
    ...
    public Bitmap clip() {
        ...
        int bitmapWidth = getWidth() - 2 * mHorizontalPadding;
        int bitmapHeight = getHeight() - 2 * mVerticalPadding;
        matrix.setScale(width / bitmapWidth, height / bitmapHeight);
        bitmap = Bitmap.createBitmap(bitmap, mHorizontalPadding,
                mVerticalPadding, bitmapWidth,
    ...
    public void setStandardRatio(float ratio, int mHorizontalPadding) {
        this.mHorizontalPadding = mHorizontalPadding;
        this.ratio = ratio;
    }


    public void setStandardSize(float width, float height) {
        this.width = width;
        this.height = height;
     ...
     */
    public void rotate() {
        postDelayed(new RotateRunnable(-90), 16);
    }


    private class RotateRunnable implements Runnable {

        private static final float deltaDegree = -10; //逆时针旋转增量

        private float dstDegree;
        private float currentDegree;

        public RotateRunnable(float degree) {
            dstDegree = degree;
        }
        ...
        public void run() {
            isAuto = true;
            float degree = 0;
            if (currentDegree + deltaDegree < dstDegree) {
                degree = dstDegree - currentDegree;
                currentDegree += degree;
            } else {
                currentDegree += deltaDegree;
                degree = deltaDegree;
            }
            mScaleMatrix.postRotate(degree, getWidth() / 2, getHeight() / 2);
            checkBorder();
            setImageMatrix(mScaleMatrix);
            if (currentDegree > dstDegree) {
                postDelayed(this, 16);
            } else {
                isAuto = false;
            }
