...
public class LineChartView extends View {
    ...
    private OnChartScorllListener mOnScorllListener;
    ...
    public interface OnChartPointClickListener {
         ...
         *
         * @param point the point nearest to clicked position in point click radius
         * @param line  the line - point holder
         */
        void onPointClick(LinePoint point, Line line);
    }

    public interface OnChartScorllListener {
        void onScrollLeft();
    ...
    private void initDefault(Context context) {
        ...
        setViewPort(50, 0, 100, 100);
    }

    /**
     * Sets the stopping force for inertial scrolling.
     *
     * @param fx the horizontal resistance
     * @param fy the vertical resistance
     */
     ...
     *
     * @param hor - horizontal limits addition
     * @param ver - vertical limits addition
     */
     ...
     *
     * @param main style for main vertical grid lines
     * @param sub  style for additional vertical grid lines
     ...
     *
     * @param main - is main vertical grid lines visible?
     * @param sub  - is additional vertical grid lines visible? (Not visible without main lines)
     * @see #setVerticalGridStyle
     */
     ...
     *
     * @param main - is main horizontal grid lines visible?
     * @param sub  - is additional horizontal grid lines visible? (Not visible without main lines)
     * @see #setHorizontalGridStyle
     */
     ...
     *
     * @param horStep          step between horizontal grid lines
     * @param horSubLinesCount count of additional lines between main horizontal lines
     * @param verStep          the step between vertical grid lines
     * @param verSubLinesCount count of additional lines between main vertical lines
     * @see #setVerticalGridStyle
     * @see #setHorizontalGridStyle
     ...
     *
     * @param hor {@link Paint Paint} for horizontal values
     * @param ver {@link Paint Paint} for vertical values
     * @see #setVerticalGridStyle
     * @see #setHorizontalGridStyle
     */
     ...
     *
     * @param hor is horizontal values visible?
     * @param ver is vertical values visible?
     ...
     *
     * @param map {@link SparseArray} with text for drawing. If <i>map</i> is not null text for lines
    ...
    public SparseArray<String> getHorValuesText() {
        return mGrid.horValuesText;
    }
     ...
     *
     * @param left   minimal horizontal value of chart to draw
     * @param bottom minimal vertical value of chart to draw
     * @param right  maximal horizontal value of chart to draw
     * @param top    maximal horizontal value of chart to draw
     */
    ...
    private void scaleCorrection() {
        mScaleX = (mViewPortRight - mViewPortLeft != 0) ? (getMeasuredWidth() - mViewPortMarginLeft - mViewPortMarginRight) / (mViewPortRight - mViewPortLeft)
                : 1;
        mScaleY = (mViewPortTop - mViewPortBottom != 0) ? (getMeasuredHeight() - LineValue.DEFAULT_Y - mViewPortMarginTop - mViewPortMarginBottom) / (mViewPortTop - mViewPortBottom)
                : 1;
    }
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (cropViewPortShader == null) {
            ...
            cropCanvas.drawRect((mViewPortMarginLeft - (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    10, getResources().getDisplayMetrics())),
                    mViewPortMarginTop - 1,
                    getWidth() - mViewPortMarginRight + 10,
    ...
    private void drawDefaultLine(Canvas canvas) {
        if (linePaint == null) {
            linePaint = new Paint();
            linePaint.setColor(0xffff8800);                    //设置画笔颜色
            linePaint.setStrokeWidth(3.0f);
        }
        canvas.drawLine(0, getHeight() - LineValue.DEFAULT_Y - 10, getWidth(), getHeight() - LineValue.DEFAULT_Y - 10, linePaint);
    }
...
//        PluLog.e("drawPointText  1111");
        if (point.getType() == LinePoint.Type.SQUARE || point.getType() == LinePoint.Type.TRIANGLE) {
            return;
        }
//        PluLog.e("drawPointText  2222");
        float x = point.getX() * mScaleX - mViewPortLeft * mScaleX + mViewPortMarginLeft;
        float y = point.getY() * (-mScaleY) + mViewPortTop * mScaleY + mViewPortMarginTop;
        if (point.isTextVisible()) {
//            PluLog.e("drawPointText  3333");
            ...
            Log.i("sssss", "point.getText()+" + point.getText() + "_______________________________txtX:" + txtX + "_____txtY:" + txtY);
            canvas.drawText(point.getText(), txtX, txtY, point.getTextPaint());

//            Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.ic_datas_currentdata_bg, new BitmapFactory.Options());
            Log.i("sssss", "________________________getHeight:" + getHeight() + "________point.getRadius():" + point.getRadius());

//            Matrix matrix = new Matrix();
//            matrix.postScale(point.getRadius() * 4 / bitmap.getWidth(),
//                    point.getRadius() * 4 / bitmap.getHeight());
//
//            Bitmap newBitmap = Bitmap.createBitmap(bitmap, 0, 0,
//                    bitmap.getWidth(), bitmap.getHeight(), matrix, true);


//            canvas.drawBitmap(newBitmap, x - point.getRadius(), getHeight()-LineValue.DEFAULT_Y/4 - point.getRadius() * 4, new Paint());
        }
    }
...
//                PluLog.e("水平字   ====" + string);
                if (string.length() > 2) {//表示显示月份
//                    canvas.drawRect(txtX - LineValue.ITEM_WIDTH / 2, txtY, txtX + LineValue.ITEM_WIDTH / 2, txtY - 40, mGrid.mainCheckReactHorValuesPaint);
                    canvas.drawText(string, txtX - LineValue.ITEM_WIDTH / 2 + 8, txtY - 8, mGrid.mainCheckHorValuesPaint);
                } else {
                    canvas.drawText(string, txtX, txtY - 8, mGrid.mainHorValuesPaint);
                }

                //Log.i("ss","______________x:"+x+"_____first:"+first+"____last:"+last);
//                if(x == 0 || x == last - 1 ){
//                    //Log.i("ss","____________________________in");
//                    canvas.drawText("", txtX, txtY, mGrid.mainHorValuesPaint);
//
//                }else {
...
//                Log.e("tag", y + "----" + "   firstHorLineY== " + firstHorLineY + "  last=" + last);
                ...
                float txtY = (getHeight() - LineValue.DEFAULT_Y - mViewPortMarginBottom) - (y - mViewPortBottom)
                        * mScaleY
                        - mGrid.mainVerValuesPaint.descent();

                if ((mGrid.verValuesAlign & TextAlign.TOP) > 0) {
                    txtY = (getHeight() - LineValue.DEFAULT_Y - mViewPortMarginBottom) - (y - mViewPortBottom)
                            * mScaleY
...
//                Log.e("tag--", txtX + "--txtX--" + "   txtY== " + txtY);

                if (StringUtil.strToInt(string, 110) <= 100 && StringUtil.strToInt(string, 110) >= 0) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                scrollToLeft = x - mLastX > 20;// mLastX < x;
                PluLog.d("action move = x=" + x + "|mLastx" + mLastX +
                        "|" + getScrollX() + "|" + getX());
            ...
            case MotionEvent.ACTION_UP:
                ...
                PluLog.d("action up x=" + x + "|velocityX=" + velocityX + "|isMovingX=" + isMovingX);
                break;
        }
        postInvalidate();
        return true;
...
//                        && gY <= p.getY() + getPointClickRadius() / mScaleY
    ...
    public void moveViewPort(float deltaX, float deltaY) {
        ...
        if (scrollToLeft) {
            if (deltaX == 0 && isMovingX == false && mOnScorllListener != null) {
                scrollToLeft = false;
                mOnScorllListener.onScrollLeft();
            }
        }
//        PluLog.d("moveViewPort deltaX" + deltaX + "}" +
//                isMovingX + "|scrollToLeft=" + scrollToLeft);
     ...
     *
     * @param listener new on point click listener
     */
     ...
     *
     * @param pointClickRadius new point click radius
     */
    ...
    public void setmOnScorllListener(OnChartScorllListener mOnScorllListener) {
        this.mOnScorllListener = mOnScorllListener;
    }
}
