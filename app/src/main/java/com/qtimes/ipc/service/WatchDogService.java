...
public class WatchDogService extends Service {
    ...
    protected final int onStart(Intent intent, int flags, int startId) {
        ...
        if (subscription != null && !subscription.isUnsubscribed()) return START_STICKY;

        //方法中调用startForeground()将服务进程设置为前台进程，
        // 当运行在 API18 以下的设备是可以直接设置，API18
        // 以上需要实现一个内部的Service，这个内部类实现和外部类同样的操作，
        // 然后结束自己；当这个服务启动后就会创建一个定时器去发送广播，
        // 当我们的核心服务被干掉后，就由另外的广播接收器去接收我们守护进程发出的广播，
        // 然后唤醒我们的核心服务；
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.N) {
            startForeground(HASH_CODE, new Notification());//设置前台进程
        ...
        subscription = Observable
                ...
                .subscribe(new SafeSubscriber<Long>(new Subscriber<Long>() {
                    @Override
